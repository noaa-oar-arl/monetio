...
def add_data(
    ...
    if has_joblib and (n_procs > 1):
        ...
        dfs = Parallel(n_jobs=n_procs, verbose=verbose)(
            delayed(_parallel_aeronet_call)(pd.DatetimeIndex([d1, d2]), **vars) for d1, d2 in zip(days, days1)
        )
...
def _parallel_aeronet_call(
    dates=None, product='AOD15', latlonbox=None, daily=False, interp_to_aod_values=None, inv_type=None, freq=None, siteid=None, detect_dust=False
):
    a = AERONET()
    df = a.add_data(
        dates,
...
class AERONET(object):
    def __init__(self):
        ...
        self.dates = [datetime.strptime('2016-06-06 12:00:00', '%Y-%m-%d %H:%M:%S'), datetime.strptime('2016-06-10 13:00:00', '%Y-%m-%d %H:%M:%S')]
        self.datestr = []
    ...
    def build_url(self):
        ...
        if self.prod in ['AOD10', 'AOD15', 'AOD20', 'SDA10', 'SDA15', 'SDA20', 'TOT10', 'TOT15', 'TOT20']:
            ...
            if self.inv_type == 'ALM15':
                ...
                inv_type = '&AML20=1'
        date_portion = (
            'year=' + sy + '&month=' + sm + '&day=' + sd + '&hour=' + sh + '&year2=' + ey + '&month2=' + em + '&day2=' + ed + '&hour2=' + eh
        )
    ...
    def read_aeronet(self):
        ...
        df = pd.read_csv(self.url, engine='python', header=None, skiprows=6, parse_dates={'time': [1, 2]}, date_parser=dateparse, na_values=-999)
