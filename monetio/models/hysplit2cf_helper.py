...
def write_with_compression(data, fname):
    ...
    for coord in ["time", "time_bounds"]:
        if coord in dset.coords:
            ...
            if "chunksizes" in existing_enc and len(existing_enc["chunksizes"]) != len(
                dset[coord].dims
            ):
                chunks = []
...
def calculate_bounds(dset):
    ...
    dset = xr.merge([dset, xr.Dataset({"bnds": ("bnds", b32)})])
...
def make_z_coordinate_cf_compliant(dset, height_reference="sea_level"):
    ...
    standard_name = (
        "height_above_mean_sea_level" if height_reference == "sea_level" else "height_above_ground"
    )
    long_name = (
        "height above mean sea level"
        if height_reference == "sea_level"
...
def make_coordinates_cf_compliant(dset):
    ...
    if "ens" in dset.coords:
        dset["ens"].attrs.update(
            {"long_name": "ensemble member", "standard_name": "realization", "units": "1"}
        )
    ...
    dset["x"].attrs.update({"long_name": "x coordinate of projection", "units": "1"})

    dset["y"].attrs.update({"long_name": "y coordinate of projection", "units": "1"})
...
def make_time_cf_compliant(dset):
    ...
    if "sampling_period_hours" not in dset.attrs:
        print('WARNING cannot make time cf compliant. sampling period not found in attributes')
        return dset
    ...
    if sampling_hours == 0:
        hours_since = [(pd.Timestamp(t) - ref_time).total_seconds() / 3600 for t in time_vals]
...
def rename_concentration_variable(
    dset, new_name, standard_name=None, long_name=None, data_var_index=0, atts={}
):
    """
    ...
    if data_var_index >= len(data_vars):
        warnings.warn(
            f"Data variable index {data_var_index} out of range. Max index is {len(data_vars)-1}"
        )
        return dset
...
def add_title_history(dset):
    if "history" not in dset.attrs:
        dset.attrs[
            "history"
        ] = f"Created on {datetime.datetime.utcnow().isoformat()} UTC by HYSPLIT NetCDF conversion script"
