def read_omps_limb(file):
    ...
    o3_error = (
        f["DataFields"]["O3VisPrecision"][:] * 1.38e-19 * temperature / pressure / (10 ** (-6))
    )
    obstime = pd.to_timedelta(
        f["GeolocationFields"]["Time"][:], unit="s"
    )  # pd.to_timedelta(obstime,unit='s')
    o3_uv = f["DataFields"]["O3UvValue"][:] * 1.38e-19 * temperature / pressure / (10 ** (-6))
    o3_uv_error = (
        f["DataFields"]["O3UvPrecision"][:] * 1.38e-19 * temperature / pressure / (10 ** (-6))
    )
    # flags
